...
fs.readdirSync(path.join(__dirname, "file-icons", "font-awesome", "svgs", "brands")).forEach((icon) => {
    ...
    let text = fs.readFileSync(path.join(__dirname, "file-icons", "font-awesome", "svgs", "brands", icon), {
        encoding: "utf8",
    });
...
fs.readdirSync(path.join(__dirname, "file-icons", "bytesize-icons", "dist", "icons")).forEach((icon) => {
    ...
    let text = fs.readFileSync(path.join(__dirname, "file-icons", "bytesize-icons", "dist", "icons", icon), {
        encoding: "utf8",
    });
...
// Write the file
fs.writeFileSync(
    path.join(__dirname, "src", "assets", "icons", "file-icons.json"),
    JSON.stringify(fileIconsObject, "", 4)
);
...
let atomConfig = CSON.parse(
    fs.readFileSync(path.join(__dirname, "file-icons", "atom", "config.cson"), { encoding: "utf8" })
);
