...
function Extract (opts) {
  ...
  outStream._write = async function(entry, encoding, cb) {
    ...
    // Defense in depth on top of the path.relative check above: also
    // require the resolved path to literally start with destination +
    // separator, closing any sibling-prefix-style bypass a string-prefix
    // check alone could miss (see SNYK-JS-UNZIPPER-18365659 / CVE-2026-17514).
    if (extractPath !== opts.path && !extractPath.startsWith(opts.path + path.sep)) {
      return cb();
    }
